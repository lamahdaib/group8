...
def search_items():
    ...
    refresh_listbox(matching)
    if len(matching) == 0:
        messagebox.showinfo("No Results", "No items found!")
...
def add_to_favourites():
    ...
    if not selection:
        ...
        return
    item = current_items[selection[0]]

    for fav in favourites:
        if fav["name"] == item["name"]:
            messagebox.showwarning("Warning", "Already in favourites!")
            return
    favourites.append(item)
    messagebox.showinfo("Added!", f"{item['name']} added to favourites!")
root.mainloop()
